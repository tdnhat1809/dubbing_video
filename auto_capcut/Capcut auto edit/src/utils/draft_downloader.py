...
def download_single_file(file_url: str, target_dir: str) -> bool:
    ...
    while retry_count <= max_retries:
        try:
            ...
            # Update draft_meta_info.json with correct draft_name and paths
            if full_file_path.endswith('draft_meta_info.json'):
                update_draft_meta_info(full_file_path, target_dir, url_draft_id)
...
def update_draft_meta_info(meta_file_path: str, target_dir: str, draft_id: str) -> bool:
    """
    Update draft_meta_info.json with correct draft_name and paths so CapCut 
    desktop correctly identifies and displays the draft.
    
    Args:
        meta_file_path: Path to draft_meta_info.json
        target_dir: Target directory where draft is saved
        draft_id: Draft ID used as the display name
    ...
    try:
        ...
        # Set draft_name to draft_id so CapCut displays it and automation can find it
        meta['draft_name'] = draft_id
        # Set correct fold_path and root_path
        ...
        safe_write_file(meta_file_path, json_content, is_binary=False)
        
        logger.info(f"Updated draft_meta_info.json: draft_name={draft_id}")
        return True
    except Exception as e:
        logger.error(f"Failed to update draft_meta_info.json: {e}")
        return False
