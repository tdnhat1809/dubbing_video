...
class DraftFolder:
    ...
    def create_draft(self, draft_name: str, width: int, height: int, fps: int = 30, *,
        ...
        # Copy template and update draft_meta_info.json with correct name/path
        meta_path = os.path.join(draft_path, "draft_meta_info.json")
        shutil.copy(assets.get_asset_path("DRAFT_META_TEMPLATE"), meta_path)
        
        # Patch draft_meta_info.json so CapCut displays correct draft name
        import json
        try:
            ...
            pass  # Non-critical, draft still works without this
