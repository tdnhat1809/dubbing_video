...
class JianyingController:
    ...
    def find_and_click_draft(self, draft_name: str, max_retries: int = 5, retry_interval: float = 5.0) -> None:
        ...
        for attempt in range(max_retries):
            try:
                # 每次重试前刷新草稿列表：切换到"模板"再切回"首页"
                if attempt > 0 or True:  # 始终刷新
    ...
    def _refresh_draft_list(self) -> None:
        """刷新草稿列表 - 通过切换tab来强制剪映重新加载草稿目录"""
        try:
            logger.info("Refreshing draft list...")
            # 方法1: 尝试点击"模板"tab再切回"首页" (支持中英文)
            template_tab = self.app.TextControl(searchDepth=2, Name="模板")
            if not template_tab.Exists(1):
                template_tab = self.app.TextControl(searchDepth=2, Name="Templates")
                
            if template_tab.Exists(1):
                template_tab.Click(simulateMove=False)
                time.sleep(2)
                home_tab = self.app.TextControl(searchDepth=2, Name="首页")
                if not home_tab.Exists(1):
                    home_tab = self.app.TextControl(searchDepth=2, Name="Projects")
                    
                if home_tab.Exists(1):
                    home_tab.Click(simulateMove=False)
                    time.sleep(3)
                    logger.info("Draft list refreshed via tab switch")
                    return
            
            # 方法2: 使用F5刷新
            pyautogui.press('f5')
            time.sleep(3)
            logger.info("Draft list refreshed via F5")
        except Exception as e:
            logger.warning(f"Failed to refresh draft list: {e}")
    ...
    def get_window(self) -> None:
        ...
        # 寻找可能存在的导出窗口 (Chinese: 导出, English: Export)
