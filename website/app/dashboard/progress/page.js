...
export default function ProgressPage() {
  ...
  // Fetch real tasks (filtered by current user)
  useEffect(() => {
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    {task.status === 'error' && (
                      <button className="px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-[11px] font-semibold text-amber-400 hover:bg-amber-400/10 transition-all" title="Thử lại"
                        onClick={async (e) => { 
                          e.stopPropagation(); 
                          try {
                            const res = await fetch(`/api/tasks/retry?id=${task.id}`, { method: 'POST' });
                            const data = await res.json();
                            if (res.ok) {
                              const retryMsg = data.retryMode === 'ocr' ? 'Retrying OCR only...' : 'Retrying full extraction...';
                              setTasks(prev => prev.map(t => t.id === task.id ? {...t, status: 'extracting', progress: 0, message: retryMsg} : t));
                            }
                    ...
                    {getStatusGroup(task.status) === 'done' && (
                      <>
                        ...
                        <button className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-amber-400 hover:bg-amber-400/10 transition-all" title="Trích xuất lại"
                          onClick={async (e) => { 
                            e.stopPropagation(); 
                            try {
                              const res = await fetch(`/api/tasks/retry?id=${task.id}&mode=full`, { method: 'POST' });
                              const data = await res.json();
                              if (res.ok) setTasks(prev => prev.map(t => t.id === task.id ? {...t, status: 'extracting', progress: 0, message: data.message || 'Re-extracting...'} : t));
                            } catch(err) { console.error(err); }
                          }}>
